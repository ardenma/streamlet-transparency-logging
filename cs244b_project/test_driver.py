...
# for i, (n, nc, flag) in tqdm(enumerate(itertools.product(num_instances, num_compromised, compromise_flags))):
#     os.makedirs(f"./logs/{n}_nodes", exist_ok=True)
#     print(f"\nStarting iteration {i}. (flag={flag}, num_compromised={nc})\n")
#     p = Popen(['./target/debug/cs244b_project', f'{n}', f'bench', f'{1}', 'small', f'{flag}', 'benchmark', f'{nc}'], stdout=sys.stdout, stderr=sys.stderr)    
#     status = p.wait()
